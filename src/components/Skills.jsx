...
const SkillCategory = ({ title, items, delayOffset }) => (
  ...
  >
    {/* Category Title */}
    <h3 className="text-xl sm:text-2xl md:text-3xl font-semibold text-accent-1 mb-6 sm:mb-8 flex items-center gap-3 sm:gap-4">
    ...
    <div className="flex flex-wrap gap-3 sm:gap-5">
      {items.map((skill, index) => (
        <motion.div
          ...
          className="flex items-center gap-2 sm:gap-4 bg-secondary-bg px-3 py-2.5 sm:px-6 sm:py-4 rounded-md border border-white/5 hover:border-accent-2/50 hover:shadow-[0_4px_20px_rgba(233,69,96,0.15)] transition-all duration-300 cursor-default"
        >
          <span className="text-xl sm:text-3xl text-accent-1">{skill.icon}</span>
...
const Skills = () => {
  return (
    <section id="skills" className="py-20 sm:py-32 bg-primary-bg relative">
